...
typedef struct {
  ...
  pthread_mutex_t *mutex;     /* Mutex protecting this Queue's data *///mutex we're locking on
  pthread_cond_t  *notFull;   /* Used by producers to await room to produce*/ //conditional variables
  pthread_cond_t  *notEmpty;  /* Used by consumers to await something to consume*/ //wont need pthread_cond_signal
  //pass pthread_cond_wait a mutex (already locked) and a pthread_cond_t
} queue;
