...
Cylinder::Cylinder(ShaderIF* sIF, float radius, const cryph::AffPoint& topCenter, cryph::AffPoint& bottomCenter, int numPoints, float r, float g, float b) : shaderIF(sIF)
{
...
//	kd[2] = checkFloat(b);
	...
	for(int i = 0; i<= numCircumferencePoints; i++)
	{
		...
		coords[j][0] = bottom.x;
		coords[j][1] = bottom.y + radius*dy;
		coords[j][2] = bottom.z + radius*dz;
		coords[j+1][0] = top.x;
		coords[j+1][1] = top.y + radius*dy;
