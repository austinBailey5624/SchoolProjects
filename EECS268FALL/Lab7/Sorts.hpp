...
template<typename T>
T* Sorts<T>::createTestArray(int size, int low, int high)
{
	NumberFileGenerator::random("nums.dat",size,low,high);
	std::ifstream myInputFile("a.out");
	int* someArray=new int[size];
	for(int i=0; i<50; i++)
	{
		myInputFile >> someArray[i];
	}
	return someArray;
}
template<typename T>
