...
data Type = TInt | TBool | TFun Type Type | TOne | TRef Type
          | TDatatype String
  deriving (Eq, Show)
...
data Expr = EInt Integer | EAdd Expr Expr | EMult Expr Expr
          ...
          | EFix Expr
          | ERef Expr | EGet Expr | EPut Expr Expr
  deriving (Eq, Show)
...
desugar dtypes (EFix e) =
    do e' <- desugar dtypes e
       return (CFix e')
desugar dtypes (ERef e) =
    CRef `fmap` desugar dtypes e
desugar dtypes (EGet e) =
    CGet `fmap` desugar dtypes e
desugar dtypes (EPut e1 e2) =
    liftM2 CPut (desugar dtypes e1) (desugar dtypes e2)
...
applp = (do k <- constructor
        ...
        (do es <- many1 atomp
            case es of
              ...
              [EVar "ref", e] -> return (ERef e)
              (EVar "get" : e : es) -> return (foldl EApp (EGet e) es)
              [EVar "put", e1, e2] -> return (EPut e1 e2)
